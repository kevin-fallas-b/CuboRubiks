...
public class ContentModel {
    ...
    private final EventHandler<MouseEvent> mouseEventHandler = event -> {
        ...
        if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
            ...
            System.out.println("camara position X: "+cameraXform.rx.getAngle());
            System.out.println("camara position Y: "+cameraXform.ry.getAngle());
            mousePosX = event.getSceneX();
            mousePosY = event.getSceneY();
        ...
        //seleccionar cual es la cara activa
        //if(mousePosX>250&&mousePosX<400)
    };
    ...
    public void moverCamara(String direccion) {
        ...
        switch (direccion) {
            ...
            case "iz":
                ...
                break;
            case "ar":
                cameraXform.setRx(-65.0);
                cameraXform.setRy(90);
                break;
            case "ab":
                cameraXform.setRx(90.0);
                cameraXform.setRy(90);
                break;
            case "ad":
                cameraXform.setRx(-30.0);
    ...
    private void setLadoActivo(){
        if(cameraXform.ry.getAngle()>=320){
            cameraXform.setRy(-40);
        }
        if(cameraXform.ry.getAngle()<=-310){
            cameraXform.setRy(50);
        }
        //if(cameraXform.rx.getAngle()>-50&&cameraXform.rx.getAngle()<30){
            if(cameraXform.ry.getAngle()>-40&&cameraXform.ry.getAngle()<50){
                if(cameraXform.rx.getAngle()>-50&&cameraXform.rx.getAngle()<30){
                   //aqui signfica que esta arriba o abajo, hacer ambas valicdaciones 
                }
                PantPrincipalController.cubo.setLadoActivo("ad");
            }else if(cameraXform.ry.getAngle()>51&&cameraXform.ry.getAngle()<135 ||cameraXform.ry.getAngle()>-310&&cameraXform.ry.getAngle()<-221){
                PantPrincipalController.cubo.setLadoActivo("de");
            }else if(cameraXform.ry.getAngle()>135&&cameraXform.ry.getAngle()<230 ||cameraXform.ry.getAngle()>-221&&cameraXform.ry.getAngle()<-130){
                PantPrincipalController.cubo.setLadoActivo("at");
            }else if(cameraXform.ry.getAngle()>230&&cameraXform.ry.getAngle()<320 || cameraXform.ry.getAngle()>-131&&cameraXform.ry.getAngle()<-41){
                PantPrincipalController.cubo.setLadoActivo("iz");
            }
